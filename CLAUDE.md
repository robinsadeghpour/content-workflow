...
| generate-content | Orchestrator that generates platform-native content for all channels from a kept idea. Produces TikTok EN, TikTok DE, Instagram, and LinkedIn drafts with voice profiles applied and critic review completed. Calls writer agents, critic agents, and the media-producer skill for visuals. Trigger: `/generate-content`. | `.claude/skills/generate-content/SKILL.md` |
...
| media-producer | Unified rendering interface for all visual output. Wraps canvas text overlays, Gemini AI images, Playwright carousels, and sharp cropping behind a single entry point. Called by generate-content to render TikTok slides, Instagram carousels, and LinkedIn carousels/infographics. | `.claude/skills/media-producer/SKILL.md` |
